...
def getMainWindow():
    ...
    try:
        mayaVersion = float(mayaVersion)
    except:
        if mayaVersion.find("Extension") != -1:
            mayaVersion = float(mayaVersion.partition(" ")[0])
    if mayaVersion > 2014:
...
class AnimationUI():
    ...
    def __init__(self):
        ...
        try:
            ...
            if mayaVersion.find("Extension") != -1:
                mayaVersion = float(mayaVersion.partition(" ")[0])
        if mayaVersion > 2014:
            self.widgets["pickerPoseToolsRadial_pasteOverTime"] = cmds.menuItem(label = "Copy/Paste Opposite over time", parent = self.widgets["pickerPoseToolsRadial"], rp = "E", c = self.copyPasteOverTime)
        ...
        for control in ["head_fk_anim", "neck_01_fk_anim", "neck_02_fk_anim", "neck_03_fk_anim", "spine_01_anim", "spine_02_anim", "spine_03_anim", "spine_04_anim", "spine_05_anim", "mid_ik_anim", "chest_ik_anim",
                        ...
                        "calf_l_twist_anim", "calf_l_twist2_anim", "calf_l_twist3_anim", "thigh_l_twist_2_anim", "thigh_l_twist_3_anim", "thigh_r_twist_2_anim", "thigh_r_twist_3_anim", "index_toe_fk_ctrl_1_l", "middle_toe_fk_ctrl_1_l", "ring_toe_fk_ctrl_1_l", "pinky_toe_fk_ctrl_1_l", "bigtoe_toe_fk_ctrl_1_l", "index_toe_fk_ctrl_1_r", "middle_toe_fk_ctrl_1_r", "ring_toe_fk_ctrl_1_r", "pinky_toe_fk_ctrl_1_r", "bigtoe_toe_fk_ctrl_1_r"]:
    ...
    def createBodyPicker(self, name, layout, *args):
        ...
        for button in[self.widgets[name + "_leftThighPickerButton"], self.widgets[name + "_leftFkKneePickerButton"], self.widgets[name + "_leftFkAnklePickerButton"], self.widgets[name + "_leftIkFootPickerButton"]]:
            ...
            cmds.menuItem(label = "Match IK Rig to current FK Pose", parent = matchMenu, c = partial(self.match_singleFrame, "leg", "l", "IK", "FK", True))
        ...
        for button in[self.widgets[name + "_rightThighPickerButton"], self.widgets[name + "_rightFkKneePickerButton"], self.widgets[name + "_rightFkAnklePickerButton"],  self.widgets[name + "_rightIkFootPickerButton"]]:
            ...
            cmds.menuItem(label = "Match IK Rig to current FK Pose", parent = matchMenu, c = partial(self.match_singleFrame, "leg", "r", "IK", "FK", True))
    ...
    def switchLegMode(self, name, mode, side, *args):
        ...
        match = cmds.menuItem(self.widgets["menuBar_settings_matching"],
                              q=True, checkBox=True)
        character = cmds.symbolButton(self.widgets["activeCharacterThumb"],
                                      q=True, ann=True)
        switch_attribute = "{0}:Rig_Settings.{1}LegMode".format(character, side)
        fk_mode = cmds.getAttr(switch_attribute)
        ...
        if mode == "IK":
            if fk_mode:
                return
            if match == True:
                self.match_singleFrame("leg", side, "FK", "IK", True)
    ...
    def returnEverything(self):
        ...
        nodes = cmds.ls(character + ":*_space_switcher_follow")
        spaceSwitchers = []
        for node in nodes:
        ...
        for control in spaceSwitchers:
            ...
            returnMe.append(spaceSwitchNode)

        for control in controls:
            if cmds.objExists(character + ":" + control):
                returnMe.append(character + ":" + control)
        ...
        for node in ["fk_orient_world_loc_l", "fk_orient_world_loc_r", "fk_orient_body_loc_l", "fk_orient_body_loc_r", "head_fk_orient_neck", "head_fk_orient_shoulder", "head_fk_orient_body", "head_fk_orient_world"]:
            if cmds.objExists(character + ":" + node):
                returnMe.append(character + ":" + node)
    ...
    def importMotion(self, *args):
        ...
        ART_importMotion.ImportMotionUI(self)
        ...
        if os.path.exists(poseCacheFile):
            ...
            for data in poseData:
                ...
                if control.find("_l") != -1:
                    ...
                    controls.append(character + ":" +ctrl)
    ...
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    def copyPasteOverTime(self, *args):
        ...
        if mayaVersion > 2014:
            from PySide import QtGui, QtCore
            parentWin = getMainWindow()

            win = QtGui.QMainWindow(parentWin)
            win.setMinimumSize(QtCore.QSize(400, 100))
            win.setMaximumSize(QtCore.QSize(400, 100))
            win.setWindowTitle("Copy/Paste Over Frame Range")
            ...
            label1 = QtGui.QLabel("Start Frame: ")
            label2 = QtGui.QLabel("End Frame: ")
            label3 = QtGui.QLabel("Frame Offset: ")

            startField = QtGui.QSpinBox()
            startField.setButtonSymbols(QtGui.QAbstractSpinBox.NoButtons)
            startField.setMinimum(-500)
            startField.setMaximum(5000)
            endField = QtGui.QSpinBox()
            endField.setButtonSymbols(QtGui.QAbstractSpinBox.NoButtons)
            endField.setMinimum(-500)
            endField.setMaximum(5000)
            offsetField = QtGui.QSpinBox()
            offsetField.setButtonSymbols(QtGui.QAbstractSpinBox.NoButtons)
            offsetField.setMinimum(-500)
            offsetField.setMaximum(5000)

            widgetLayout.addWidget(label1)
            widgetLayout.addWidget(startField)
            widgetLayout.addWidget(label2)
            widgetLayout.addWidget(endField)
            widgetLayout.addWidget(label3)
            widgetLayout.addWidget(offsetField)

            # layout
            process_layout = QtGui.QHBoxLayout()
            layout.addLayout(process_layout)

            # widgets
            self.use_keys = QtGui.QCheckBox("Use Keys")
            button = QtGui.QPushButton("Process")
            button.setMinimumWidth(300)

            # add to layout
            process_layout.addWidget(self.use_keys)
            process_layout.addWidget(button)

            # signals
            button.clicked.connect(partial(self.copyPasteOverTime_Process, startField, endField, offsetField))

            #get start frame end frame
            start = cmds.playbackOptions(q = True, min = True)
            end = cmds.playbackOptions(q = True, max = True)
    ...
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    def copyPasteOverTime_Process(self, start, end, offset, *args):

        #get field values

        startFrame = start.value()
        endFrame = end.value()
        offsetAmt = offset.value()

        selection = cmds.ls(sl = True)
        key_frames = cmds.keyframe(selection, q=True)
        ...
        for i in range(startFrame, endFrame + 1):
            ...
            # only use keys from selection
            time = cmds.currentTime(i)
            if self.use_keys.isChecked():
                if time not in key_frames:
                    continue

            self.copyPose()
            controls = self.pasteOppositePreview()
            self.pastePoseOpposite()
        ...
        cmds.selectKey()
        cmds.keyframe(edit = True, r = True, o = "over", tc = offsetAmt)
    ...
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    def ikHeelSolve(self, character, side, start, mid, end, *args):


        angleDim = cmds.shadingNode("angleDimension", asUtility = True)
        cmds.connectAttr(start + ".translate", angleDim + ".startPoint")
        cmds.connectAttr(mid + ".translate", angleDim + ".middlePoint")
        cmds.connectAttr(end + ".translate", angleDim + ".endPoint")

        currentAngle = cmds.getAttr(angleDim + ".angle")

        if currentAngle > 1:
            cmds.setAttr(character + ":heel_ctrl_" + side + ".rotateZ", currentAngle * -1)
            cmds.setKeyframe(character + ":heel_ctrl_" + side + ".rotateZ")

        cmds.delete(cmds.listRelatives(angleDim, parent = True)[0])
        cmds.delete(start)
        cmds.delete(mid)
        cmds.delete(end)
    ...
    def match_frameRange_UI_Process(self, *args):
        ...
        #solve roll?
        solveRoll = cmds.checkBox(self.widgets["matchFR_RollSolveCB"], q = True, v = True)
        ...
        for i in range(start, end + 1):
            ...
            for part in parts:
                if part == character + ":" + "Right Leg":
                    if method == "fk":
                        self.match_singleFrame("leg", "r", "FK", "IK", solveRoll)
                    if method == "ik":
                        self.match_singleFrame("leg", "r", "IK", "FK", solveRoll)


                if part == character + ":" + "Left Leg":
                    if method == "fk":
                        self.match_singleFrame("leg", "l", "FK", "IK", solveRoll)
                    if method == "ik":
                        self.match_singleFrame("leg", "l", "IK", "FK", solveRoll)
    ...
    def match_frameRange_UI(self, *args):
        ...
        self.widgets["matchFR_RollSolveCB"] = cmds.checkBox(label = "Solve Foot Roll", v = True, parent = self.widgets["matchFrameRange_MainLayout"])
    ...
    def setupButtonAttrsOnControls(self):
        ...
        for character in characters:
            ...
            try:
                if cmds.objExists(character + ":" + "fk_clavicle_l_anim.buttonName"):
                    cmds.setAttr(character + ":" + "fk_clavicle_l_anim.buttonName",self.widgets[character + "_leftClavPickerButton"], type = "string")

                else:
                    cmds.select(character + ":" + "fk_clavicle_l_anim")
                    cmds.addAttr(ln = "buttonName", dt = "string", keyable = False)
                    cmds.setAttr(character + ":" + "fk_clavicle_l_anim.buttonName",self.widgets[character + "_leftClavPickerButton"], type = "string")
            except:
                pass
            ...
            try:
                if cmds.objExists(character + ":" + "fk_clavicle_r_anim.buttonName"):
                    cmds.setAttr(character + ":" + "fk_clavicle_r_anim.buttonName",self.widgets[character + "_rightClavPickerButton"], type = "string")

                else:
                    cmds.select(character + ":" + "fk_clavicle_r_anim")
                    cmds.addAttr(ln = "buttonName", dt = "string", keyable = False)
                    cmds.setAttr(character + ":" + "fk_clavicle_r_anim.buttonName",self.widgets[character + "_rightClavPickerButton"], type = "string")
            except:
                pass
