...
class AutoRigger():
    ...
    def buildIKLegs_hind(self):
        ...
        for side in ["l", "r"]:
            ...
            cmds.select(clear = True)
            beginJoint = cmds.joint(name = "noflip_begin_joint_" + side)
            #beginJoint = cmds.duplicate(ikThighJoint, name = "noflip_begin_joint_" + side, parentOnly = True)[0]
            cmds.select(clear = True)
            endJoint = cmds.joint(name = "noflip_end_joint_" + side)
            #endJoint = cmds.duplicate(ikFootJoint, name = "noflip_end_joint_" + side, parentOnly = True)[0]
            cmds.select(clear = True)
            cmds.setAttr(beginJoint + ".v", 0)

            beginPos = cmds.xform(ikThighJoint, q = True, ws = True, piv = True)
            cmds.xform(beginJoint, ws = True, t = (beginPos[0], beginPos[1], beginPos[2]))
            endPos = cmds.xform(ikFootJoint, q = True, ws = True, t = True)
            cmds.xform(endJoint, ws = True, relative = True, t = (endPos[0], endPos[1], endPos[2]))
            cmds.parent(endJoint, beginJoint)
            cmds.makeIdentity(beginJoint, t = 0, r = 1, s = 0, apply = True)
            ...
            constraint = cmds.pointConstraint(beginJoint, targetGrp)[0]
            cmds.delete(constraint)
            const = cmds.aimConstraint("calf_" + side, targetGrp, weight=1, mo=False, aimVector=(0, -1, 0), upVector=(0, 0, -1), worldUpType="object", worldUpVector=(0, 0, -1), worldUpObject="foot_" + side, sk=["x", "y"])
            cmds.delete(const)
            cmds.parent(targetLoc, targetGrp)
            constraint = cmds.pointConstraint(endJoint, targetLoc)
            cmds.delete(constraint)
            ...
            const = cmds.aimConstraint("calf_" + side, noflipVectorLoc, weight=1, mo=False, aimVector=(0, -1, 0), upVector=(0, 0, 1), worldUpType="object", worldUpVector=(0, 0, 1), worldUpObject="thigh_" + side, sk=["x"])
            cmds.delete(const)

            if side == "l":
                #cmds.setAttr(noflipVectorLoc + ".ty", (-200 * scale))
                cmds.move(0, (-200 * scale), 0, noflipVectorLoc, r=True, os=True)
            else:
                #cmds.setAttr(noflipVectorLoc + ".ty", (200 * scale))
                cmds.move(0, (-200 * scale), 0, noflipVectorLoc, r=True, os=True)
            ...
            cmds.parentConstraint(endJoint, noflipVectorGrp, mo=True)
            ...
            footIK = ikNodesLeg[0]
            poleVecY = cmds.getAttr(footIK+".poleVectorY")

            cmds.refresh()
            rotX = cmds.getAttr(ikThighJoint+".rx")
            cmds.setAttr(footIK+".twist", rotX)
            ...
            #create pole vector constraint between knee loc and full ik leg rp ik handle
            if poleVecY > 0:
                cmds.move(0, (400 * scale), 0, noflipVectorLoc, r=True, os=True)
            ...
            #clean up the hierarchy
            ctrlGrp = cmds.group(name = "leg_ctrl_grp_" + side, empty = True)
            cmds.parent([targetGrp, aimGrp, noflipVectorGrp], ctrlGrp)
            legGroup = cmds.group(name = "leg_group_" + side, empty = True)
            constraint = cmds.pointConstraint("driver_pelvis", legGroup)[0]
            cmds.delete(constraint)
            cmds.parent([footCtrlSpaceSwitcherFollow, beginJoint, ctrlGrp], legGroup)
            ...
            cmds.addAttr(longName=("knee_twist"), at = 'double', keyable = True)
            if side == "r":
                twistPlusMinusNode = cmds.shadingNode("plusMinusAverage", name = "ik_leg_" + side + "_plusMinusAverageNode", asUtility = True)
                cmds.connectAttr(footCtrl + ".knee_twist", twistPlusMinusNode + ".input1D[0]")
                cmds.setAttr(twistPlusMinusNode+".input1D[1]", -rotX)
                cmds.connectAttr(twistPlusMinusNode + ".output1D", footIK + ".twist")
                ...
                cmds.connectAttr(footCtrl + ".knee_twist", twistPlusMinusNode + ".input1D[0]")
                cmds.setAttr(twistPlusMinusNode+".input1D[1]", -rotX)
                cmds.connectAttr(twistPlusMinusNode + ".output1D", twistMultNode + ".input1X")
                cmds.setAttr(twistMultNode + ".input2X", -1)
                cmds.connectAttr(twistMultNode + ".outputX", footIK + ".twist")
    ...
    def finishLegs_hind(self):
        ...
        for side in ["l", "r"]:
            ...
            if side == "r":
                sourceConn = cmds.listConnections("invis_legs_foot_ikHandle_" + side + ".twist", c=True, p=True)
