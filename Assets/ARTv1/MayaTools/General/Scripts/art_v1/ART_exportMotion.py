...
class ExportMotionUI():
    ...
    def __init__(self):
        ...
        self.widgets["exportMotion_SelExportNode"] = cmds.button(label="Select Settings Node", w = 140, h = 50,  parent = self.widgets["leftSideButtonColumn"], c = "cmds.select(\"ExportAnimationSettings\")")
        self.widgets["exportMotion_saveSettings"] = cmds.button(label="Save Settings", w = 140, h = 50,  parent = self.widgets["leftSideButtonColumn"], c = self.saveExportInfo)
        ...
        self.widgets["exportFBX_fastExportCB"] = cmds.checkBox(label = "Fast Export", v = False, parent = self.widgets["exportFBXFormLayout"])
        self.widgets["exportFBX_exportMorphsCB"] = cmds.checkBox(label = "Export Morphs", v = True, parent = self.widgets["exportFBXFormLayout"], onc = self.findMorphs, ofc = self.clearMorphs)
        self.widgets["exportFBX_exportCustomAttrsCB"] = cmds.checkBox(label = "Export Custom Attrs", v = False, parent = self.widgets["exportFBXFormLayout"], onc = self.findAttrs, ofc = self.clearAttrs)

        cmds.formLayout(self.widgets["exportFBXFormLayout"], edit = True, af = [(self.widgets["exportFBX_fastExportCB"], "top", 5), (self.widgets["exportFBX_fastExportCB"], "left", 370)], ann = "FAST EXPORT IS FOR ITERATION ONLY!!!")
        cmds.formLayout(self.widgets["exportFBXFormLayout"], edit = True, af = [(self.widgets["exportFBX_exportMorphsCB"], "top", 40), (self.widgets["exportFBX_exportMorphsCB"], "left", 370)])
        cmds.formLayout(self.widgets["exportFBXFormLayout"], edit = True, af = [(self.widgets["exportFBX_exportCustomAttrsCB"], "top", 195), (self.widgets["exportFBX_exportCustomAttrsCB"], "left", 370)])

        self.widgets["exportFBX_morphsList"] = cmds.textScrollList(w = 165, h = 120, parent = self.widgets["exportFBXFormLayout"], allowMultiSelection=True)
        cmds.formLayout(self.widgets["exportFBXFormLayout"], edit = True, af = [(self.widgets["exportFBX_morphsList"], "top", 58), (self.widgets["exportFBX_morphsList"], "left", 370)])

        self.widgets["exportFBX_attrsList"] = cmds.textScrollList(w = 165, h = 120, parent = self.widgets["exportFBXFormLayout"], allowMultiSelection=True)
        cmds.formLayout(self.widgets["exportFBXFormLayout"], edit = True, af = [(self.widgets["exportFBX_attrsList"], "top", 215), (self.widgets["exportFBX_attrsList"], "left", 370)])
        ...
        if cmds.objExists("ExportAnimationSettings") == False:
            ...
            try:
                ...
                cmds.checkBox((self.widgets["exportFBX_fastExportCB"]), edit = True, v = settings[0])
                cmds.checkBox((self.widgets["exportFBX_exportMorphsCB"]), edit = True, v = settings[1])
                cmds.checkBox((self.widgets["exportFBX_exportCustomAttrsCB"]), edit = True, v = settings[2])

                if settings[1] == True:
                    cmds.textScrollList(self.widgets["exportFBX_morphsList"], edit = True, da = True)
                    cmds.textScrollList(self.widgets["exportFBX_morphsList"], edit = True, si = settings[3])
                if settings[2] == True:
                    self.findAttrs()
                    cmds.textScrollList(self.widgets["exportFBX_attrsList"], edit = True, da = True)
                    cmds.textScrollList(self.widgets["exportFBX_attrsList"], edit = True, si = settings[4])
            except:
                pass
    ...
    def exportFBX(self,  *args):
        ...
        fastExport = cmds.checkBox((self.widgets["exportFBX_fastExportCB"]), q = True, v = True)
            ...
            if fileName != "":
                ...
                for joint in joints:
                    ...
                    if parent != None:
                        if parent[0] in joints:
                            ...
                            cmds.connectAttr(character + ":" + parent[0] + "|" + character + ":" + joint + ".scale", joint + ".scale")
                        ...
                        if joint == "root":
                            ...
                            constraints.append(constraint)
                            cmds.connectAttr( character + ":" + joint + ".scale", joint + ".scale")

                            if not fastExport:

                                exportAttrs = cmds.checkBox((self.widgets["exportFBX_exportCustomAttrsCB"]), q = True, v = True)
                                ...
                                if exportAttrs == True:
                                    ...
                                    #selected attrs in list
                                    attrsToExport = cmds.textScrollList(self.widgets["exportFBX_attrsList"], q = True, selectItem = True)
                ...
                if startFrame != "":
                    ...
                    if cmds.objExists("custom_export_shapes"):
                        cmds.select("custom_export_shapes", add = True)
                    ...
                    if not fastExport:
                        mel.eval("keyTangent -global -itt linear;")
                        mel.eval("keyTangent -global -ott linear;")
                        cmds.bakeResults(simulation = True, t = (startFrame, endFrame))
                        cmds.delete(constraints)
                        mel.eval("keyTangent -global -itt auto;")
                        mel.eval("keyTangent -global -ott auto;")

                        #getting the connection info on each joint's scale and then breaking it
                        for joint in joints:
                            try:
                                info = cmds.connectionInfo(joint + ".scale", sourceFromDestination = True)
                                cmds.disconnectAttr(info, joint + ".scale")
                            except:
                                pass
                        ...
                        #do a morph fix
                        if cmds.objExists("custom_export_shapes"):
                            cmds.selectKey("custom_export_shapes")
                    ...
                    newpath = fileName.replace(fileName.split("/")[-1], "")
                    ...
                    if cmds.optionMenu(self.widgets["exportFBX_weaponList"], q = True, v = True) != "No Weapon":
                        ...
                        constraints = []
                        for joint in joints:
                            constraint1 = cmds.parentConstraint(namespace + ":" + joint, joint)[0]
                            constraint2 = cmds.scaleConstraint(namespace + ":" + joint, joint)[0]
                        ...
                        for attr in [".tx", ".ty", ".tz", ".rx", ".ry", ".rz"]:
                            #cmds.cutKey(root + attr)
                            cmds.setAttr(root + attr, 0)

                        #kvassey adding to force root scale to 1
                        for attr in [".sx", ".sy", ".sz"]:
                            cmds.setAttr(root + attr, 1)
    ...
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    def saveWarning(self,  *args):

        cmds.warning("Please remember to save your export settings!")
    ...
    def saveExportInfo(self,  *args):
        ...
        animInfo = []
        for child in children:
            height = cmds.formLayout(child, q = True, h = True)
            if height > 1:
                index = children.index(child)
                animInfo.append([child, index])
        ...
        #save checkbox and selection settings
        fastExport = cmds.checkBox((self.widgets["exportFBX_fastExportCB"]), q = True, v = True)
        exportMorphs = cmds.checkBox((self.widgets["exportFBX_exportMorphsCB"]), q = True, v = True)
        exportAttrs = cmds.checkBox((self.widgets["exportFBX_exportCustomAttrsCB"]), q = True, v = True)


        selectedMorphs = cmds.textScrollList(self.widgets["exportFBX_morphsList"], q = True, selectItem = True)
        selectedAttrs = cmds.textScrollList(self.widgets["exportFBX_attrsList"], q = True, selectItem = True)


        cmds.addAttr(exportAnimSettings, dt = "string", ln = "settings")


        jsonString = json.dumps([fastExport, exportMorphs, exportAttrs, selectedMorphs, selectedAttrs])
        cmds.setAttr(exportAnimSettings + ".settings", jsonString, type = "string")


        #save out anim sequence data
        i = 0
        for each in animInfo:
            index = each[1]
            #create a string attr with sequenceName|startFrame|endFrame|FPS
            string = str(cmds.textField(self.widgets["addAnimSequence_" + str(index + 1) + "_TextField"], q = True, text = True)) + "::"
            string += str(cmds.intField(self.widgets["addAnimSequence_" + str(index + 1) + "_startField"], q = True, v = True)) + "::"
            string += str(cmds.intField(self.widgets["addAnimSequence_" + str(index + 1) + "_endField"], q = True, v = True)) + "::"
            string += cmds.optionMenu(self.widgets["addAnimSequence_" + str(index + 1) + "_fpsField"], q = True, v = True) + "::"
            string += cmds.optionMenu(self.widgets["addAnimSequence_" + str(index + 1) + "_interpMenu"], q = True, v = True) + "::"
            string += character
        ...
        for each in self.deleteLayouts:
            cmds.deleteUI(each)
    ...
    def removeAnim(self, layout,  *args):
        ...
        cmds.formLayout(layout, edit = True, h = 1)
        self.deleteLayouts.append(layout)
    ...
    def addAnim(self, layout, mode,  *args):
        ...
        #add the textField and browse button
        self.widgets["addAnimSequence_" + str(numChildren + 1) + "_TextField"] = cmds.textField(w = 290, h = 30, cc = partial(self.saveWarning))
        ...
        self.widgets["addAnimSequence_" + str(numChildren + 1) + "_startField"] = cmds.intField(v = 0, w = 60, cc = partial(self.saveWarning))
        self.widgets["addAnimSequence_" + str(numChildren + 1) + "_endField"] = cmds.intField(v = 0, w = 60, cc = partial(self.saveWarning))
    ...
    def findMorphs(self, *args):
        ...
        for shape in blendshapes:
            niceName = shape.rpartition(":")[2]

            if niceName not in physiqueShapes:
                cmds.textScrollList(self.widgets["exportFBX_morphsList"], edit = True, append = shape)

        allItems = cmds.textScrollList(self.widgets["exportFBX_morphsList"], q = True, ai = True)
        cmds.textScrollList(self.widgets["exportFBX_morphsList"], edit = True, si = allItems)
    ...
    def findAttrs(self, *args):
        ...
        for attr in customAttrs:
            cmds.textScrollList(self.widgets["exportFBX_attrsList"], edit = True, append = attr)

        allItems = cmds.textScrollList(self.widgets["exportFBX_attrsList"], q = True, ai = True)
        cmds.textScrollList(self.widgets["exportFBX_attrsList"], edit = True, si = allItems)
    ...
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
    def clearAttrs(self, *args):
        cmds.textScrollList(self.widgets["exportFBX_attrsList"], edit = True, removeAll = True)
