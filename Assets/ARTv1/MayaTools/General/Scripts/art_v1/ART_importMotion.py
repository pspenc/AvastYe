...
class ImportMotionUI():
    ...
    def importMocap(self, *args):
        ...
        if not os.path.exists(filePath):
            ...
            if cmds.objExists("root"):
                cmds.warning("There is already a skeleton in the scene with the name \"root\".")
                prompt = cmds.confirmDialog(title = "Root Exists", message = "Do you want to remove errant skeleton?",button=['Yes','No'], defaultButton='Yes', cancelButton='No', dismissString='No')
                if prompt == "Yes":
                    cmds.delete("root")
                else:
                    return
    ...
    def importMocap_IK(self, character, head, body, leftArm, rightArm, leftLeg, rightLeg, rootMotion):
        ...
        if leftArm == True:
            ...
            cmds.pointConstraint("clavicle_l", character + ":clavicle_l_anim", mo = True)
        ...
        if rightArm == True:
            ...
            cmds.pointConstraint("clavicle_r", character + ":clavicle_r_anim", mo = True)
    ...
    def importMocap_FK(self, character, head, body, leftArm, rightArm, leftLeg, rightLeg, rootMotion):
        ...
        if body == True:
            ...
            #constraints
            cmds.parentConstraint("pelvis", character + ":body_anim", mo = True)

            cmds.orientConstraint("spine_01", character + ":spine_01_anim", mo = True)
            cmds.orientConstraint("spine_02", character + ":spine_02_anim", mo = True)

            if cmds.objExists(character + ":spine_03_anim"):
                cmds.orientConstraint("spine_03", character + ":spine_03_anim", mo = True)

            if cmds.objExists(character + ":spine_04_anim"):
                cmds.orientConstraint("spine_04", character + ":spine_04_anim", mo = True)

            if cmds.objExists(character + ":spine_05_anim"):
                cmds.orientConstraint("spine_05", character + ":spine_05_anim", mo = True)


        if head == True:
            cmds.orientConstraint("neck_01", character + ":neck_01_fk_anim", mo = True)

            if cmds.objExists(character + ":neck_02_fk_anim"):
                cmds.orientConstraint("neck_02", character + ":neck_02_fk_anim", mo = True)

            if cmds.objExists(character + ":neck_03_fk_anim"):
                cmds.orientConstraint("neck_03", character + ":neck_03_fk_anim", mo = True)


            cmds.orientConstraint("head", character + ":head_fk_anim", mo = True)
