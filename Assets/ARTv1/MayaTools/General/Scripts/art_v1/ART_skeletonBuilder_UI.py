...
class SkeletonBuilder_UI():
    ...
    def __init__(self):
                ...
                if utils.attrExists('Skeleton_Settings.faceModule'):
                    ...
                    for f in faceDict.keys():
                        if cmds.objExists(f):
                            ...
                            cmds.error('Cannot find the faceModule node named ' + f + '\nFace modules in scene: ' + str(modules))
            except Exception, e:
        ...
        cmds.refresh()
        cmds.dockControl(self.widgets["dock"], r=True, e=True)
    ...
    def dynamicJointInfo_UI(self, *args):
        ...
        #create the window and the layouts
        window = cmds.window("dynamicJointInfo_UI", w = 300, h = 185, title = "Joint Info", tbm = False, sizeable = True)
        mainLayout = cmds.columnLayout(w = 300, h = 185)
        formLayout = cmds.formLayout(w = 300, h = 185, parent = mainLayout)
    ...
    def dynamicJointInfo_Accept(self, *args):
        ...
        if not num.isdigit() and num != "":
            return cmds.confirmDialog(title = "Error", icon = "warning", message = "Please use numbers in number of joints")
        if num == "":
            return cmds.confirmDialog(title = "Error", icon = "warning", message = "Please input the amount of joints in the chain.")
    ...
    def extraJointInfo_UI_GetJoints(self, layout, *args):
        ...
        if legStyle == "Hind Leg":
            returnList.append("thigh_l")
            returnList.append("calf_l")
            returnList.append("heel_l")
            returnList.append("foot_l")
            returnList.append("thigh_r")
            returnList.append("calf_r")
            returnList.append("heel_r")
            returnList.append("foot_r")
        ...
        fingerFields = [["leftHandOptions_numThumbBones", "thumb_l"],["leftHandOptions_numIndexBones", "index_l"], ["leftHandOptions_numMiddleBones", "middle_l"], ["leftHandOptions_numRingBones", "ring_l"], ["leftHandOptions_numPinkyBones", "pinky_l"], ["rightHandOptions_numThumbBones", "thumb_r"],["rightHandOptions_numIndexBones", "index_r"], ["rightHandOptions_numMiddleBones", "middle_r"], ["rightHandOptions_numRingBones", "ring_r"], ["rightHandOptions_numPinkyBones", "pinky_r"]]

        for field in fingerFields:
    ...
    def faceInfo_UI(self, *args):
        ...
        #create the window and the layouts
        window = cmds.window('faceInfo_UI', w = 305, h = 185, title = 'Face Module Setup', tbm = False, sizeable = True)
        mainLayout = cmds.columnLayout(w = 305, h = 185)
        formLayout = cmds.formLayout(w = 305, h = 185, parent = mainLayout)

        #widgets
        label1 = cmds.text(label = "Enter Face Module Name:    ")
        label2 = cmds.text(label = "Choose Face Attachment Joint: ")

        name = cmds.textField("faceInfo_UI_moduleName", w = 145)
        attachJoint = cmds.optionMenu("faceInfo_UI_attachJoint", w = 125)
        ...
        #place widgets
        cmds.formLayout(formLayout, edit = True, af = [(label1, 'top', 10), (label1, 'left', 5)])
        cmds.formLayout(formLayout, edit = True, af = [(label2, 'top', 45), (label2, 'left', 5)])

        cmds.formLayout(formLayout, edit = True, af = [(name, 'top', 10), (name, 'left', 150)])
        cmds.formLayout(formLayout, edit = True, af = [(attachJoint, 'top', 45), (attachJoint, 'left', 170)])

        cmds.formLayout(formLayout, edit = True, af = [(okButton, 'top', 125), (okButton, 'left', 5)])
        cmds.formLayout(formLayout, edit = True, af = [(cancelButton, 'top', 125), (cancelButton, 'left', 150)])

        # create the facial rig selector label
        label_facialRigWin = cmds.text(label='Base Facial Rig:')

        faceRigWidgetWin = "faceInfo_UI_baseRig"
        self.widgets[faceRigWidgetWin] = cmds.optionMenu(w=290, parent=formLayout)

        # get the base rig files from disk
        for mayaFile in self.availableBaseRigs():
            if mayaFile == "faceMask_low.ma":
                cmds.menuItem(label=mayaFile, parent=self.widgets[faceRigWidgetWin])

        cmds.formLayout(formLayout, edit=True, af=[(label_facialRigWin, "top", 70), (label_facialRigWin, "left", 5)])
        cmds.formLayout(formLayout, edit=True, af=[(self.widgets[faceRigWidgetWin], "top", 90), (self.widgets[faceRigWidgetWin], "left", 5)])
    ...
    def faceInfo_UI_Accept(self, *args):
        ...
        baseFaceRigFile = cmds.optionMenu(self.widgets["faceInfo_UI_baseRig"], q = True, value = True)
        cmds.deleteUI("faceInfo_UI")
        ...
        #create the parent label used for the face module
        label_parent = cmds.text(label = "Parent: ")
        ...
        #create the facial rig selector label
        label_facialRig = cmds.text(label = 'Facial Rig:')

        faceRigWidget = "rigMod_" + faceModuleName + "_settings_baseRig"
        self.widgets[faceRigWidget] = cmds.optionMenu(w=150, parent=self.widgets["rigMod_" + faceModuleName + "_settingsForm"])

        #get the base rig files from disk
        for mayaFile in self.availableBaseRigs():
            cmds.menuItem(label=mayaFile, parent=self.widgets[faceRigWidget])

        #set option menu to the choice from the faceInfo_UI
        if baseFaceRigFile:
            cmds.optionMenu(self.widgets[faceRigWidget], e=1, v=baseFaceRigFile)
        else:
            cmds.warning('ART_skeletonBuilder_UI>> Face Module: No face rigs found in path!')
        ...
        cmds.formLayout(self.widgets[faceForm], edit=True, af=[(label_facialRig, "top", 70), (label_facialRig, "left", 5)])
        cmds.formLayout(self.widgets[faceForm], edit=True, af=[(self.widgets[faceRigWidget], "top", 90), (self.widgets[faceRigWidget], "left", 5)])
    ...
    def build(self, *args):
        ...
        #Builds the joint mover hierarchy
        ...
        if result == "Yes":
            ...
            #Delete the skeleton settings and cache nodes
            if cmds.objExists("SkeletonSettings_Cache"):
            ...
            #add current info attrs to settings
                                ...
                                if type == "face":
                                    ...
                                    if cmds.attributeQuery('faceModule', node="Skeleton_Settings", ex=1):
                                        ...
                                        if name not in existingFaces.keys():
                                            existingFaces[name] = {'parent':parent, 'rig':baseRig}
                                        ...
                                        faceRigWidget = "rigMod_" + name + "_settings_baseRig"
                                        baseRig = cmds.optionMenu(self.widgets[faceRigWidget], q=1, v=1)
                                        facialData = {}
                                        print 'baseRig:',baseRig
                                        facialData[name] = {'parent':parent, 'rig':baseRig}
                        ...
                        if parent != "":
                            if type != "":
                                ...
                                if type == "face":
                                    ...
                                    # check if face exists, if so load it and add to it, else add the attr and save the info
                                    # this just stores face name keys and parent joint pairs
                                    if cmds.attributeQuery('faceModule', node=skeletonSettingsTemplateNode, ex=1):
                                        existingFaces = json.loads(cmds.getAttr(skeletonSettingsTemplateNode + '.faceModule'))
                                        if name not in existingFaces.keys():
                                            existingFaces[name] = {'parent': parent, 'rig': baseRig}
                                        cmds.setAttr(skeletonSettingsTemplateNode + '.faceModule', json.dumps(existingFaces),
                                                     type='string')
                                    else:
                                        cmds.addAttr(skeletonSettingsTemplateNode, longName="faceModule", dt='string')
                                        faceRigWidget = "rigMod_" + name + "_settings_baseRig"
                                        baseRig = cmds.optionMenu(self.widgets[faceRigWidget], q=1, v=1)
                                        facialData = {}
                                        facialData[name] = {'parent': parent, 'rig': baseRig}
                                        cmds.setAttr(skeletonSettingsTemplateNode + '.faceModule', json.dumps(facialData),
                                                     type='string')
    ...
    def edit(self, *args):
        ...
        #IMPORTANT: This method fires when you go back from skeleton layout to skeleton creation

        print 'INFO: Fired the \'edit\' method in ART_skeletonBuilder_UI'
        ...
        #list of items to delete/unlock
        nodesToDelete = ["preview_root"]
        ...
        for parent in ["root", "lra", "middle_01_l_lra1", "geo_mover", "root_mover_grp", "Proxy_Geo_Grp", "extra_joints_dynamic", "extra_joints_template", "extra_joints_dynamic_geo", "extra_joints_template_geo", "character_facing_direction", "JointMover"]:
            try:
                nodesToDelete.extend(cmds.listRelatives(parent, ad=1, f=1))
            except:
                print 'Exception at line 5146, unlocking joint movers:', parent

        nodesToDelete.extend("Skeleton_Settings")

        #unlocking joint mover and layout hierarchies
        ...
        ## facial block to read from skeletonSettings_Cache
        ###############################################################
        faceDict = {}
        try:
            # get facial module info
            if utils.attrExists('SkeletonSettings_Cache.faceModule'):
                # get the name and attachment point from skeletonSettings
                faceDict = json.loads(cmds.getAttr('SkeletonSettings_Cache.faceModule'))

            #delete the constraints and reparent
            for fm in faceDict.keys():
                currentFace = face.FaceModule(faceNode=fm)
                # find the attach mover in this face
                attachMe = None
                for jm in currentFace.jointMovers:
                    if 'faceAttach' in jm:
                        attachMe = cmds.listRelatives(jm, parent=1)[0]
                if attachMe:
                    cmds.lockNode(attachMe, lock=0)
                    #store the prev location, in case it jumps
                    utils.deleteConstraints(attachMe)
                    #TODO: This won't work with multiple faces during a setup phase, need to abstract facial topnodes later for multi-face characters
                    try:
                        cmds.parent(attachMe, 'jointMovers', a=1)
                    except:
                        print 'Already parented', attachMe

                    #unconstrain the mask
                    #TODO: this won't work with a character that has multiple faces during setup
                    utils.deleteConstraints('mask_pivot_back_GRP')
                #remove facial jms from the list of nodes ot be deleted at the end of this method
                cmds.select(attachMe, hi=True)
                facialJms = cmds.ls(sl=1)
                cmds.select(cl=1)
                for fjmNode in facialJms:
                    if fjmNode in nodesToDelete:
                        nodesToDelete.pop(nodesToDelete.index(fjmNode))
        except Exception, e:
            cmds.warning("Facial Module not included.")
            ...
            if widget.find("rigMod_") == 0:
                if widget.find("_settingsForm") != -1:
                    try:
                        ...
                        print 'Widget does not exist: ' + str(widget)
        ...
        if faceDict:
            ...
            for fm in faceDict.keys():
                ...
                faceModuleName = fm
                faceParent = faceDict[fm]['parent']
                baseFaceRigFile = faceDict[fm]['rig']
                ...
                # create the name field
                self.widgets["rigMod_" + faceModuleName + "_settings_name"] = cmds.textField(w=100, parent=self.widgets["rigMod_" \
                                                                                                                        + faceModuleName + "_settingsForm"], text=faceModuleName,
                                                                                             cc=partial(self.updateRigModuleList, faceModuleName, "face"))
                ...
                # set that label
                self.widgets["rigMod_" + faceModuleName + "_settings_parent"] = cmds.textField(w=80, parent=self.widgets["rigMod_" + \
                                                                                                                         faceModuleName + "_settingsForm"], text=faceParent, editable=False)
                self.widgets["rigMod_" + faceModuleName + "_settings_changeParent"] = cmds.button(label="...", w=20, parent= \
                    self.widgets["rigMod_" + faceModuleName + "_settingsForm"], ann="Change Attachment Joint Mover",
                                                                                                  c=partial(self.extraJointInfo_UI_ParentList, faceModuleName))
                ...
                # attaching the widgets to the form
                cmds.formLayout(self.widgets[faceForm], edit=True, af=[(self.widgets["rigMod_" + \
                                                                                     faceModuleName + "_settings_name"], "top", 5),
                                                                       (self.widgets["rigMod_" + faceModuleName + "_settings_name"], "right", 5)])
                cmds.formLayout(self.widgets[faceForm], edit=True, af=[(label_parent, "top", 35), (label_parent, "left", 5)])
                cmds.formLayout(self.widgets[faceForm], edit=True, af=[(self.widgets["rigMod_" + \
                                                                                     faceModuleName + "_settings_parent"], "top", 35),
                                                                       (self.widgets["rigMod_" + faceModuleName + "_settings_parent"], "right", 25)])
                cmds.formLayout(self.widgets[faceForm], edit=True, af=[(self.widgets["rigMod_" + \
                                                                                     faceModuleName + "_settings_changeParent"], "top", 35),
                                                                       (self.widgets["rigMod_" + faceModuleName + "_settings_changeParent"], "right", 5)])
        ...
        nodesToDelete.append("JointMover")
        #so that there's no spam below when deleting this locked container's children before it's deleted.
        ...
        for node in nodesToDelete:
            if cmds.objExists(node):
                try:
                    ...
                    print 'Exception in line 5551 for node:', node
    ...
    def jointMoverUI_resetAll(self, *args):
        ...
        print 'INFO: ENTERING jointMoverUI_resetAll'
    ...
    def createRigPose_execute(self, skinWeight, *args):
        ...
        for mover in self.globalMovers:
            ...
            #don't add facial joint movers to the rig pose
            try:
    ...
    def getSkeletonModelPose(self):
        ...
        for joint in joints:
            if joint.find("twist") == -1:
                ...
                #ignore facial joints
                try:
                    if utils.attrExists(joint + '.uType'):
                        continue
                except Exception, e:
                    cmds.warning("Facial Module not included.")
    ...
    def setModelPose_JM(self):
        print 'INFO: firing setModelPose_JM'
        if cmds.iconTextCheckBox(self.widgets["symmetry"], q = True, v = True):
            cmds.iconTextCheckBox(self.widgets["symmetry"], e = True, v = False)
            self.jointMoverUI_SymmetryModeOff()
    ...
    def lock_phase1(self, *args):
        ...
        print 'INFO: ART PHASE >>> RUNNING LOCK PHASE1'
    ...
    def lock_phase1b(self, result, *args):
        print 'INFO: ART PHASE >>> RUNNING LOCK PHASE1B'
    ...
    def lock_phase2(self, deleteProxy, skinWeight, *args):
        ...
        print 'INFO: ART PHASE >>> RUNNING LOCK PHASE2'
        ...
        for mover in self.geoMovers:
            ...
            if len(cmds.ls(mover)) > 1:
                cmds.warning('NAME CLASH: Mover: ' + mover)
    ...
    def unlock(self, *args):
        try:
            ...
            self.activateMasks()
            #rebuild the constraints gluing the topnodes of the SDKs to the LRAs
            for f in utils.getUType('faceModule'):
                currentFace = face.FaceModule(faceNode=f)
                currentFace.faceMask.buildSdkParentConstraints()
        except Exception, e:
            cmds.warning("Facial Module not included.")
    ...
    def publish(self, project, characterName, handCtrlSpace, *args):
        print 'ENTERING PUBLISH CODE BLOCK'
        # enable logger
        _errors = list()
        logger = Logger()

        if self.logging:
            logging_checkbox = cmds.checkBox(self.widgets["loggerUICheckbox"], q=True, value=True)
            if logging_checkbox:
                # external
                from epic.internal import epic_logger
                logger = epic_logger.EpicLogger()
                logger.terminal()
        ...
        try:
            ##FACIAL
            # must run before rig in case of spine pop
            import maya.mel as mel
            if utils.attrExists('Skeleton_Settings.faceModule'):
                for f in utils.getUType('faceModule'):
                    currentFace = face.FaceModule(faceNode=f)

                    # flush the eye sdks offsets
                    mel.eval(
                        '''
                        parentConstraint -e -maintainOffset L_eye_LRA  L_eye_SDK_grp_parentConstraint1;
                        parentConstraint -e -maintainOffset R_eye_LRA  R_eye_SDK_grp_parentConstraint1;
                        '''
                    )

                    # TODO: this needs to get set back to 1 when stepping back
                    cmds.setAttr(currentFace.node + '.lraToSdkConstraints', 0.0)
        ...
        try:
            ...
            if utils.attrExists('Skeleton_Settings.faceModule'):
                ...
                if utils.attrExists(currentFace.node + '.controlBoardFile'):
                    # hookup control board
                    for f in utils.getUType('faceModule'):
                        currentFace = face.FaceModule(faceNode=f)

                        # query the sdks and hide them
                        cmds.hide(currentFace.sdks)

                        # import the facial board
                        boardPath = os.path.dirname(face.__file__) + '/' + currentFace.controlBoardFile
                        cmds.file(boardPath, i=1, type='mayaAscii', pr=1)
                        # TODO: this next block won't work with multi-headed characters, need markup on controlboard
                        currentFace.connectControlBoard('controlBoard')

                        #if the placer exists, use it
                        if cmds.objExists('control_board_placer'):
                            cmds.delete(cmds.parentConstraint('control_board_placer', 'face_ctrl_grp'))
                        cmds.parentConstraint('head', 'face_ctrl_grp', mo=1)

                        # rig up the eye aim to blend as an intermediary between the controlboard and blendboard
                        cmds.createNode('blendWeighted', name='look_left_BW')
                        cmds.createNode('blendWeighted', name='look_right_BW')
                        cmds.createNode('blendWeighted', name='look_up_BW')
                        cmds.createNode('blendWeighted', name='look_down_BW')
                        mel.eval(
                            '''
                            connectAttr -f blendBoard_look_right.output look_right_BW.input[0];
                            connectAttr -f controlBoard.look_right look_right_BW.input[1];
                            connectAttr -f look_right_BW.output blendBoard.look_right;
                            connectAttr -f eyes_ctrl.use_eye_aim look_right_BW.weight[0];

                            connectAttr -f blendBoard_look_left.output look_left_BW.input[0];
                            connectAttr -f controlBoard.look_left look_left_BW.input[1];
                            connectAttr -f look_left_BW.output blendBoard.look_left;
                            connectAttr -f eyes_ctrl.use_eye_aim look_left_BW.weight[0];

                            connectAttr -f blendBoard_look_up.output look_up_BW.input[0];
                            connectAttr -f controlBoard.look_up look_up_BW.input[1];
                            connectAttr -f look_up_BW.output blendBoard.look_up;
                            connectAttr -f eyes_ctrl.use_eye_aim look_up_BW.weight[0];

                            connectAttr -f blendBoard_look_down.output look_down_BW.input[0];
                            connectAttr -f controlBoard.look_down look_down_BW.input[1];
                            connectAttr -f look_down_BW.output blendBoard.look_down;
                            connectAttr -f eyes_ctrl.use_eye_aim look_down_BW.weight[0];
                            '''
                        )
                        if cmds.objExists('mid_eye_aim_driver_pointConstraint1'):
                            cmds.delete('mid_eye_aim_driver_pointConstraint1')
                        # attach the eye aim control to the master control
                        cmds.parentConstraint('master_anim', 'eye_aim_CTRL_grp', mo=1)
                else:
                    cmds.warning('FACE PREDATES CONTROLBOARD MARKUP: ' + currentFace.node)
        except Exception, e:
            cmds.warning("Facial Module not included.")
        ...
        # report
        logger.summary_header()
        if _errors:
            logger.error(_errors, exc_info=True, pretty=True, title="ERRORS")
        else:
            logger.info("RIG BUILT WITHOUT ERRORS", title="SUCCESS")
    ...
    def publish_UI(self, *args):
        ...
        self.widgets["publishUIPublishBtn"] = cmds.button(w = 136, h = 50, label = "Build", c = self.publish_UI_TakeScreenshot, parent = self.widgets["publishUIButtonRow"], ann = "Build the character rig with current settings.")
        self.widgets["publishUICancelBtn"] = cmds.button(w = 136, h = 50, label = "Cancel", c = self.publish_UI_Cancel, parent = self.widgets["publishUIButtonRow"])
        self.widgets["loggerUICheckbox"] = cmds.checkBox(label="Enable Logging", h=40, w=300, parent=self.widgets["publishUIMainLayout"])
        try:
            from epic.internal import epic_logger
            self.logging = True
        except ImportError:
            self.logging = False
            cmds.checkBox(self.widgets["loggerUICheckbox"], e=True, en=False)
    ...
    def buildFinalSkeleton(self):
        ...
        try:
            for f in utils.getUType('faceModule'):
                ...
                #these are rebuilt when stepping backwards, see line 7686
        except Exception, e:
            cmds.warning("Facial Module not included.")
        ...
        for mover in self.geoMovers:
            ...
            if children != None or mover.find("ball") == 0:
                ...
                if self.attrExists(mover + '.lra'):
                    ...
                    cmds.select(clear = True)
                    joint = cmds.joint(name = jointName)
                    cmds.addAttr(joint, longName='uType', dt='string')
                    cmds.setAttr(joint + '.uType', 'facialJoint', type='string')
    ...
    def skinTvFilter(self, tvList, mesh, vtx, skin, remap={}):
        #checks that the transform exists
        returnList = []

        for tvPair in tvList:
            t = tvPair[0]
            v = tvPair[1]
            if len(cmds.ls(t)) > 1:
                cmds.warning('skinTvFilter: multiple transforms with name: ' + t + ', influence culled. Mesh: ' + mesh + ' vtx: ' + str(vtx))
                #TODO: offer the user to choose using code below
                continue

            elif cmds.objExists(t) == False:

                #doing this here as to not add time per vtx when joints exist
                tvDict = {}
                for pair in tvList:
                    tvDict[pair[0]] = pair[1]

                # check if the joint is already remapped
                if t in remap.keys():
                    if remap[t] != t:

                        #check to see if the remapped joint is already in the tvList (ugh.. I apologize)
                        if remap[t] in tvDict.keys():
                            tvPair = [remap[t], v + tvDict[remap[t]]]
                            #update original and new lists, not knowing if this was parsed yet
                            for i in range(0, len(tvList)):
                                if tvList[i][0] == remap[t]:
                                    tvList[i] = tvPair

                            for i in range(0, len(returnList)):
                                if returnList[i][0] == remap[t]:
                                    returnList[i] = tvPair

                        else:
                            tvPair = [remap[t], v]
                            returnList.append(tvPair)
                else:
                    #check that it's maya 2014+ or they have pyside
                    #this is a check in case the joint wasn't already remapped
                    try:
                        if utils.module_exists('PySide'):
                            from epic.utils import userItemSelect

                            items = cmds.ls('*', type='joint')
                            # no PySide
                            userSelectedJoint = userItemSelect.show(items, title='Remap Influence: ' + t + ' for Mesh: ' + mesh, message='The influence [' + t + '] cannot be found,\nwould you like to remap this influence for all remaining verts on mesh [' + mesh + ']?.')
                            if userSelectedJoint:
                                remap[t] = userSelectedJoint
                                returnList.append([userSelectedJoint, v])

                                #if the remapped transform is not in the skinCluster, add it
                                if userSelectedJoint not in cmds.skinCluster(skin, q=1, inf=1):
                                    cmds.skinCluster(skin, e=1, ai=userSelectedJoint,lw=True,wt=0.0)
                                    cmds.setAttr(userSelectedJoint+".liw",0)
                            else:
                                #remap it to itself to not bug the user again
                                remap[t] = t
                    except Exception, e:
                        cmds.warning("Facial Module not included.")

                    cmds.warning('skinTvFilter: no transform found with name: ' + t + ', influence culled. Mesh: ' + mesh + ' - user offered Remap')
            else:
                returnList.append(tvPair)
        return returnList, remap


    def loadCachedWeights(self, debug=0):
        ...
        for file in files:
            ...
            if cmds.objExists(niceName):
                ...
                if importAll == False:
                    result = cmds.confirmDialog(icon = "question",
                                                title = "Import Skin Weights",
                                                message = "Skin Weight Data was found for " + niceName + ".",
                                                button = ["Import", "Import All", "Ignore", "Ignore All"],
                ...
                if result == "Import":
                    ...
                    #look for joints or remap them when not found
                    joints = []
                    remapDict = {}
                    for jnt in transforms:
                        if cmds.objExists(jnt):
                            if cmds.nodeType(jnt):
                                joints.append(jnt)
                            else:
                                cmds.warning('ART_SkeletonBuilder_UI: loadCacheWeights>>> Transform is not a joint: ' + jnt)
                        else:
                            # check that it's maya 2014+ or they have pyside
                            if jnt not in remapDict.keys():
                                try:
                                    if utils.module_exists('PySide'):
                                        from epic.utils import userItemSelect

                                        items = cmds.ls('*', type='joint')
                                        userSelectedJoint = userItemSelect.show(items,
                                                                                title='INFLUENCE NOT FOUND!',
                                                                                message='JOINT: ' + jnt + '   MESH: ' + niceName + '\nCannot find skinning influence, would you like to remap to another joint?',
                                                                                comboSelection=lastSelected)
                                        if userSelectedJoint:
                                            joints.append(userSelectedJoint)
                                            #update the remapDict
                                            remapDict[jnt] = userSelectedJoint
                                            lastSelected = userSelectedJoint
                                        else:
                                            #the user has said no to remapping, let's not bother him again, setting this joint to be remapped to itself means it won't be remapped
                                            remapDict[jnt] = jnt
                                            cmds.warning('ART_SkeletonBuilder_UI: loadCacheWeights>>> Joint not found: ' + jnt)
                                except Exception, e:
                                    cmds.warning("Facial Module not included.")
                    ...
                    if joints:
                        try:
                            cmds.select(cl=1)
                            for jnt in joints:
                                if cmds.lockNode(jnt, q=1):
                                    cmds.lockNode(jnt, lock=False)
                            skin = cmds.skinCluster(joints, niceName, toSelectedBones=1, skinMethod = 0, name = (niceName +"_skinCluster"))[0]
                        except Exception as e:
                            print 'ART_SkeletonBuilder_UI: loadCacheWeights>>> Cannot make skinCluster: mesh:', niceName, 'jnts:', str(joints)
                    ...
                    if skin:
                        ...
                        for vtx in skinDict.keys():
                            ...
                            #set the weight for each vertex
                            try:
                                #skinTvFilter will update the remapDict if needed
                                tvList, remapDict = self.skinTvFilter(skinDict[vtx], niceName, vtx, skin, remap=remapDict)
                                cmds.skinPercent(skin, niceName + ".vtx[" + str(vtx) + "]", transformValue= tvList)
                            except Exception as e:
                                exc_type, exc_obj, exc_tb = sys.exc_info()
                                fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                                print(exc_type, fname, exc_tb.tb_lineno)
                                print 'ART_skeletonBuilder_UI>>>> loadCachedWeights: skinDict[vtx] = ', skinDict[vtx], '\n'

                        #normalize in case remapping no longer adds to 1.0 (when you're remapping to something already influencing the same vert)
                        if remapDict:
                            cmds.skinPercent(skin, niceName, normalize=1)
                        end = time.time()
                        print 'COMPLETED: loadCacheWeights>>>', verts, 'vertex weights loaded onto', niceName, 'in ', str(end - start), 'seconds.'
                        #close progress window
                        cmds.progressWindow(endProgress = 1)
    ...
    #########################################
    #CHRIS' FACIAL AREA - KEEP OUT
    #########################################

    ## BUTTON / UI CODE
    #########################################

    def availableBaseRigs(self, debug=0):
        rigs = []
        dirPath = os.path.dirname(face.__file__)
        print '>>>ART_skeletonBuilder_UI: Searching for base facial rigs in:', dirPath
        for file in os.listdir(dirPath):
            if 'false' not in file:
                if 'faceMask' in file:
                    rigs.append(file)
                    if debug:
                        print '>>>ART_skeletonBuilder_UI: FOUND:', file
        return rigs
    ...
    def constrainSdksToHeadControl(self):
        #TODO: This won't work with multiple faces
        cmds.parentConstraint('head', 'sdkNodes', mo=1)
        cmds.scaleConstraint('head', 'sdkNodes', mo=1)
    ...
    ########################################
    ## END FACIAL ART 'STATE' CODE

def skeleton_builder_toggle():
    if cmds.dockControl("skeletonBuilder_dock", exists=True):
       channelBox = cmds.formLayout("SkelBuilder_channelBoxFormLayout", q=True,
                             childArray=True)
       if channelBox != None:
          channelBox = channelBox[0]
          ...
          # edit the channel box pane's attachment to the formLayout
          cmds.formLayout(channelBoxLayout, edit=True,
                      af=[(channelBox, "left", 0),
                         (channelBox, "right", 0),
                         (channelBox, "bottom", 0)],
          attachControl=(channelBox, "top", 0, channelBoxForm))
          cmds.deleteUI("skeletonBuilder_dock")
    else:
     SkeletonBuilder_UI()

class Logger(object):
    """Remove Try and Except for Logger exception.
        NOTE: This way is much less expensive than a bunch of expection handles.
    """
    def error(self, message=None, exc_info=True, pretty=True, title=str()):
        pass

    def info(self, message=None, title=str()):
        pass
